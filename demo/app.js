...
function selo(status) {
  ...
  return `<span class="inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium ${s.classe}">${esc(s.rotulo)}</span>`
}

function cartao(titulo, valor, rodape) {
  return `
    <div class="rounded-lg border bg-white p-5 shadow-sm">
      <p class="text-sm text-slate-500">${esc(titulo)}</p>
      <p class="mt-1 text-2xl font-semibold">${esc(valor)}</p>
      ${rodape ? `<p class="mt-1 text-sm text-slate-500">${rodape}</p>` : ''}
    </div>`
}
...
function cabecalho(titulo, descricao) {
  return `
    <div class="mb-6">
      ...
      ${descricao ? `<p class="mt-1 text-sm text-slate-500">${esc(descricao)}</p>` : ''}
    </div>`
}

function emConstrucao(texto) {
  return `<div class="rounded-lg border border-dashed border-slate-300 bg-white p-6 text-sm text-slate-600">${texto}</div>`
}
...
function telaCatalogo() {
  ...
  const cards = visiveis
    .map(
      (p) => `
      <div class="flex flex-col overflow-hidden rounded-lg border bg-white shadow-sm">
        <div class="flex aspect-[4/3] items-center justify-center bg-slate-100 text-xs text-slate-400">
          sem foto
        </div>
        <div class="flex flex-1 flex-col p-4">
          <span class="w-fit rounded-md border px-2 py-0.5 text-xs text-slate-600">${esc(p.categoria)}</span>
          <h3 class="mt-2 font-medium">${esc(p.nome)}</h3>
          <p class="mt-1 line-clamp-2 text-xs text-slate-500">${esc(p.descricao)}</p>
          <div class="mt-auto pt-3">
            <p class="font-medium">${p.tipoValor === 'medio' ? 'a partir de ' : ''}${brl(p.valor)}</p>
            ${
              p.estoque !== null && p.estoque !== undefined
                ? `<p class="text-xs text-slate-500">${p.estoque} em estoque</p>`
                : ''
            }
          </div>
        </div>
  ...
  return `
    ...
    <div class="mb-6 flex flex-wrap gap-2">
      ...
      <select id="categoria" class="h-9 rounded-md border border-slate-300 bg-white px-3 text-sm">
        <option value="">Todas as categorias</option>${opcoes}
      </select>
    </div>
    ${
      visiveis.length === 0
        ? '<p class="text-sm text-slate-500">Nenhum presente encontrado com esses filtros.</p>'
  ...
  return `
    ...
    <ol class="space-y-3">
      ${etapas
        .map(
          ([nome, texto], i) => `
        <li class="flex gap-4 rounded-lg border bg-white p-4">
          <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-900 text-sm font-medium text-white">${i + 1}</span>
          <div>
            <p class="font-medium">${esc(nome)}</p>
            <p class="mt-0.5 text-sm text-slate-500">${esc(texto)}</p>
          </div>
        </li>`,
        )
        .join('')}
...
function telaMinhas() {
  ...
  return `
    ...
    <div class="mb-6 rounded-lg border bg-white p-5 shadow-sm">
      <p class="text-sm text-slate-500">Gasto no mês</p>
      <p class="mt-1 text-2xl font-semibold">${brl(gasto)}</p>
      <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-slate-100">
        <div class="h-full bg-emerald-500" style="width:${pct}%"></div>
      </div>
      <p class="mt-2 text-sm text-slate-500">${brl(gasto)} de ${brl(limite)} — canceladas e devolvidas não entram na conta.</p>
    </div>
...
function telaGestao() {
  ...
  return `
    ...
    <div class="mb-6 flex flex-wrap items-center gap-2">
      <input placeholder="Código, cliente ou consultor" class="h-9 w-56 rounded-md border border-slate-300 px-3 text-sm" />
      <input type="date" class="h-9 rounded-md border border-slate-300 px-3 text-sm" />
      <select class="h-9 rounded-md border border-slate-300 bg-white px-3 text-sm"><option>Todos os consultores</option></select>
      <select class="h-9 rounded-md border border-slate-300 bg-white px-3 text-sm">
      ...
      <span class="ml-auto flex gap-2">
        <span class="rounded-md border bg-white px-3 py-1.5 text-sm text-slate-600">Exportar CSV</span>
        <span class="rounded-md border bg-white px-3 py-1.5 text-sm text-slate-600">Exportar XLSX</span>
      </span>
    </div>
...
function telaDetalhe() {
  ...
  const itens = s.itens
    ...
    <tr class="border-b last:border-0">
      <td class="p-3">
        ${esc(i.produto)}
        ${i.site ? `<a href="${esc(i.site)}" target="_blank" rel="noopener" class="block truncate text-xs text-slate-500 underline underline-offset-2">${esc(i.site)}</a>` : ''}
      </td>
  ...
  const historico = s.historico
    ...
    <div class="border-l-2 border-slate-200 pl-3 text-sm">
      ...
      <p class="text-xs text-slate-500">${esc(h.quando)} · ${esc(h.quem)}</p>
      ${h.motivo ? `<p class="mt-1 text-xs text-slate-700">${esc(h.motivo)}</p>` : ''}
    </div>`,
    )
    .join('')
  ...
  return `
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500">${esc(s.data)} · ${esc(s.consultor)}</p>
      </div>
      <div class="flex items-center gap-3">
        ${selo(s.status)}
        <select id="trocaDetalhe" class="h-9 rounded-md border border-slate-300 bg-white px-2 text-sm">${outras}</select>
      </div>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
      <div class="space-y-6 lg:col-span-2">
        ...
        <div class="rounded-lg border bg-white p-5">
          ...
          <p class="mt-1 text-sm text-slate-500">Motivo: ${esc(s.motivo)}</p>
      ...
      <div class="space-y-6">
        <div class="rounded-lg border bg-white p-5">
          ...
          <p class="text-sm text-slate-500">${esc(s.cpf)}</p>
        </div>
        <div class="rounded-lg border bg-white p-5">
          <p class="font-medium">Entrega</p>
          <p class="mt-2 text-sm">${esc(s.destinatario)}</p>
          <p class="text-sm text-slate-500">${esc(s.entrega)}</p>
        </div>
        <div class="rounded-lg border bg-white p-5">
          <p class="mb-2 font-medium">Alterar status</p>
          ${
            perfil === 'consultor'
              ? '<p class="text-sm text-slate-500">Seu perfil acompanha o status, mas não o altera.</p>'
              : `<p class="text-sm text-slate-500">A partir de <strong>${esc(STATUS[s.status].rotulo)}</strong> a solicitação pode ir para:</p>
...
function telaCompras() {
  ...
  const linhas = naFila
    ...
    <tr class="border-b last:border-0 hover:bg-slate-50">
      ...
      <td class="p-3">
        ${esc(i.produto)}
        <div class="text-xs text-slate-500">${esc(i.consultor)} · para ${esc(i.cliente)}</div>
      </td>
      <td class="max-w-xs p-3">
        ${
          i.site
            ...
            : '<span class="rounded-md border px-2 py-0.5 text-xs text-slate-500">catálogo</span>'
        }
      </td>
      <td class="p-3 text-right">${i.quantidade}</td>
...
function telaProdutos() {
  const linhas = PRODUTOS.map(
    (p) => `
    <tr class="border-b last:border-0 ${p.ativo ? '' : 'text-slate-400'}">
...
function telaUsuarios() {
  const linhas = CONSULTORES.map(
    ...
    <tr class="border-b last:border-0">
      ...
      <td class="p-3"><span class="rounded-md bg-slate-100 px-2 py-0.5 text-xs">${esc(u.perfil)}</span></td>
    ...
    .map(
      ...
      <button data-tela="${t.id}"
        class="rounded-md px-2.5 py-1 text-sm transition-colors ${
          t.id === tela ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'
        }">${esc(t.rotulo)}</button>`,
    )
    .join('')
